...
var _ = Describe("JobManager", func() {
	Describe("Sync", func() {
		...
		var (
			...
			jobhelper *jobHelper.MockJobHelper
		)
		...
		BeforeEach(func() {
			...
			jobhelper = jobHelper.NewMockJobHelper(ctrl)
		})
		...
		DescribeTable("should return the correct status depending on the job status",
			func(s batchv1.JobStatus, r jobHelper.Result, expectsErr bool) {
				j := batchv1.Job{
			...
			Entry("active", batchv1.JobStatus{Active: 1}, jobHelper.Result{Requeue: true, Status: jobHelper.StatusInProgress}, false),
			Entry("active", batchv1.JobStatus{Active: 1}, jobHelper.Result{Requeue: true, Status: jobHelper.StatusInProgress}, false),
			Entry("succeeded", batchv1.JobStatus{Succeeded: 1}, jobHelper.Result{Status: jobHelper.StatusCompleted}, false),
			Entry("failed", batchv1.JobStatus{Failed: 1}, jobHelper.Result{}, true),
		)
		...
		It("should create the job and return without error if it doesn't exist", func() {
			...
			Expect(
				...
				Equal(jobHelper.Result{Requeue: true, Status: jobHelper.StatusCreated}),
			)
		})
		...
		It("should delete the job if it was edited", func() {
			...
			Expect(
				...
				Equal(jobHelper.Result{Requeue: true, Status: jobHelper.StatusInProgress}),
			)
		})
	})
})
